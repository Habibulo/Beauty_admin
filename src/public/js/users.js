...
document.addEventListener('DOMContentLoaded', () => {
    // Initialize the correct background color for each select on page load
    document.querySelectorAll('.status-active').forEach(select => {
      const selectedOption = select.querySelector('option:checked');
      const statusClass = selectedOption.value;
  
      // Apply the correct class based on the status
      select.classList.add(statusClass);
  
      // Event listener for when the status is changed
      select.addEventListener('change', async (e) => {
        const id = e.target.id;
        const status = e.target.value;
  
        // Update the background color dynamically based on the selected status
        e.target.className = `status-active ${status}`; // Set new class for background color
  
        // Make API call to update the member status in the backend
        try {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    pdf_btn.onclick = async () => {
        await toPDF(customers_table);
    }
